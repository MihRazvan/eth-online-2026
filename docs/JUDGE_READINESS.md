...
| [ ] | J15 / Graph | Deploy the actual Subgraph and consume live Substreams | Generate manifest from deployed address/start block, authenticate provider, stream sufficient initialized history, persist/resume cursor and test undo handling. Requires T01/T02 |
...
| [ ] | T01 | Make Graph access available | Studio project/slug and deploy access, Graph Market JWT token, query access where required. Put credentials in local/hosting secrets, not chat; Codex configures, deploys and verifies |
| [ ] | T02 | Select an always-on host and operator | Existing Railway/Render/Fly/VPS account, or approve a concrete proposed paid plan after costs are known. Identify primary and fallback operator and how alerts reach them. Vercel frontend hosting alone is insufficient |
