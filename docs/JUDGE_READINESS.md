...
| [ ] | J15 / Graph | Deploy the actual Subgraph and consume live Substreams | Studio v0.1.0 deployed from actual address/start block; live hash-pinned query verified. Still authenticate Substreams, stream sufficient initialized history, persist/resume cursor and test undo handling. Requires remaining T01/T02; [evidence](evidence/graph.md) |
...
| [ ] | T01 | Make Graph access available | Studio project/slug/deploy access received and deployment verified. Remaining: Graph Market API Token (JWT) as `SUBSTREAMS_API_TOKEN` in ignored `.env`/hosting secrets. Studio query currently needs no extra credential. Codex connects and verifies the stream |
