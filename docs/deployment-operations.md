...
The settlement service is now hosted on Railway and connected to the Vercel frontend. At the 13 September readiness check, protocol pins, the enabled keeper, authenticated proof acquisition and the private backup roundtrip all passed. These operational probes do **not** establish a checkpoint for an actual public sale, disaster restoration of its endpoint witness, or a completed participant lifecycle. Those receipts remain separate acceptance gates. New commitments always require fresh readiness.

## Provisioned infrastructure

The isolated Railway project is `usufruct` (`cb65063e-ebf1-452c-b077-5e3da03041e2`), production environment `b60dbcfa-a033-4afb-b7c7-4371610c9a27`. It was created empty; the existing `second-order` project is unrelated and remains untouched. The configuration is recorded in [.railway/railway.ts](../.railway/railway.ts). Its initial plan contained exactly three resources:
...
| `endpoint-data` | 500 MB persistent volume mounted at `/data`; SQLite journals and two local proof copies |
...
Estimated initial usage is **$5–15/month**, subject to measured CPU, memory and traffic. At continuously saturated configured compute ceilings, compute alone is about $25/month, plus storage/egress. Monitored project operating budget: **$30/month**. This is a monitored budget, not an account-wide hard limit. On 13 September the user delegated the hosting choice after a Vercel/Render/Railway cost comparison. The isolated service, 500 MB volume and private bucket are now provisioned. The original 1,024 MB instance patch failed the current account's 500 MB limit; a reviewed exact-ID repair reused the same resources. No account plan upgrade or duplicate resources were needed. The project cost monitor warns without stopping settlement. [Railway resource pricing](https://docs.railway.com/pricing/plans).
...
The prepared keeper address is `0xFfaa5fE1C38Aa538fd9B1A28D8CF8516fa3728Ad`. It received 0.01 Sepolia ETH in a canonical finalized transaction and is enabled with its original persistent journal. [Independent funding readback](evidence/operations/keeper-funding-readback.json). No series checkpoint had been submitted at that observation. The default transaction cap is0.0024testETH; rolling24h reserved spend cap0.01testETH; fixed80000gas maximum; gas-price cap30gwei, priority cap1gwei. These are ceilings, not expected per-checkpoint costs. The actual isolated checkpoint test used45729gas. The key cannot be used by the HTTP API; the keeper code only prepares zero-value calls to the pinned checkpoint contract.

## Deployment sequence

1. Retain the exact reviewed source commit and image digest. The bounded hosting choice and initial resources are already authorized and provisioned. Review any subsequent infrastructure plan against the existing IDs; do not replay the original create plan or upgrade the account.
...
The prepared USDC/WETH pool initialized at Sepolia block7432532. A measured1,000-block historical replay took50.252seconds and used1,040,384database bytes; replaying the full4.26million-block prefix would require roughly4.43GB before additional overhead. **Do not run that full replay on the proposed500 MB shared volume.** Instead, the default history starts at an independently verified actual Swap at block11678801, before any current deployment sale. This retains a known initial tick and complete subsequent earning-window coverage, while explicitly excluding earlier pool history. Analysis rejects a series activated before its selected pool's anchor. The stream checks free disk before opening the database and at most every100responses, stopping if less than256MiB remains. This periodic guard reserves operating room but is not a hard quota against concurrent writers. Hosted catch-up has run under the 512 MiB ceiling; retain fresh memory/disk and canonical-tail observations during operation. See [Graph service evidence](evidence/graph-service.md). Additional pools need their own verified anchor and retained coverage; the selected demo pool does not cover every discovered NFT.

## Operational enablement and shutdown repair

The initial disabled keeper database used a legacy configuration fingerprint that included the operational `enabled` flag. Enabling it therefore failed closed with `KEEPER_CONFIG_CHANGED`. The reviewed migration accepts only exact legacy configurations with the same normalized network, signer, deployment, gas, budget and scheduling policy; it excludes only enablement from the new binding. It updates one metadata field and preserves jobs, signed bytes, receipts and reservations. Disabled startup clears persisted readiness synchronously, and the gateway independently requires configured enablement. Eighteen keeper unit tests, an actual isolated-chain checkpoint/restart/reorg test and36operations/verifier tests pass.

A stale Graph writer lock left by the stopped failed container was archived only after checking the exact old deployment, hostname, PID, creation time, disabled replacement and absence of any competing stream process. The existing history database was preserved. Normal restarts must release locks without manual recovery; never erase a database to fix a configuration or lock failure.

The subsequent normal-redeploy test identified duplicate SIGTERM delivery: the parent signalled children on shutdown and again in `finally`, while one-shot child handlers made the second signal fatal. The corrected parent signals once, waits for concurrent child exits for30seconds, bounds HTTP draining and stops advertising new-sale readiness immediately. Keeper and stream handlers tolerate repeated signals until cleanup. Idle waits are abortable; in-flight signing policy remains unchanged. The Railway manifest and explicit runtime setting retain a45-second deployment drain.

[The verified normal restart](evidence/operations/hosted-restart.json) reopened the existing databases, reacquired both locks without manual recovery and resumed indexed history. [Fresh public readiness](evidence/operations/hosted-readiness.json) passed after the restart. Slow or stuck RPC work can still exceed graceful-shutdown limits; a forced crash requires the documented exclusive offline recovery, never automatic cross-host lock stealing. Actual series checkpointing and remote endpoint restoration await the controlled public lifecycle.
