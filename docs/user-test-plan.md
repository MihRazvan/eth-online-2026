...
Use this plan to test whether an LP, a claim buyer and a maker can understand and complete FeeStrip's actual workflows. The economic reference is the [product contract](../feestrip-handoff/PRODUCT_CONTRACT.md); priorities come from the [product research](research/product-landscape.md). This is a test plan, not a claim that public deployment, wallet testing or live analytics already passed. Check [current status](STATUS.md) before recording a result.
...
- The retention worker running before N, with configured proof access and the independent validator. Follow [retention setup and limits](../packages/settlement/README.md). Record responsibility for the separate permissionless checkpoint transaction; the worker does not sign it.

For the public pass, the actual outstanding access is a configured, funded Sepolia deployment signer; participant wallets with test assets; a suitable RPC with demonstrated historical-proof access or a measured endpoint-acquisition strategy; and a verified deployment. Configuration paths or keystore aliases are sufficient handoff information. Private keys, recovery phrases and tokens stay out of session notes, recordings and chat.
