...
These are actual browser renders of a component mounted by the test with simulated API observations. They demonstrate layout and state language, not live service availability, genuine historical proof retention, source finality or chain acceptance. The lead owns the separate integrated real-service/browser lifecycle evidence. No shared chain or backend was mutated by this UI lane.
