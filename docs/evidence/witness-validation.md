...
- Canonical integer encodings in account/storage RLP. A stored RLP zero leaf is rejected; ordinary Ethereum zero values come from authenticated absence. Optional retained manager bytecode and declared value arrays, when present, must agree with authenticated data.
