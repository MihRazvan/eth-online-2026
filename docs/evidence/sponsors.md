...
| The Graph | Studio v0.2.0 indexes the replacement deployment and matches RPC ([current query](operations/graph-upgrade-verification.json)); authenticated Substreams v0.1.1 verifies real mint decoding/cursor resume and earlier common-block agreement with Studio v0.1.0 ([stream evidence](substreams.md)); no live buyer result yet | Not a substitute for live-provider evidence | Subgraph build; reusable imported Substreams module, WASM/sink and common-block join tests | Actual sale-event mapping, initialized history, hosted joined buyer query and UI result; reusable package/example now public |
...
The standalone retention worker saves evidence; the separate restricted keeper submits checkpoints. Both are implemented, but the combined hosted preservation service and remote backup remain unoperated. A canonical anchor must be preserved within N+256 unless growth has already been verified and cached. Hosted operation and fallback must precede public acceptance. Two directories on the same host do not establish independent durability.
