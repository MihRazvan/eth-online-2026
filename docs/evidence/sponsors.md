...
| The Graph | Studio v0.1.0 + authenticated Substreams v0.1.1 share a block/hash matching RPC; real mint decoding and cursor resume verified ([evidence](substreams.md)); no live buyer result yet | Not a substitute for live-provider evidence | Subgraph build; reusable imported Substreams module, WASM/sink and common-block join tests | Actual sale-event mapping, initialized history, hosted joined buyer query and UI result; reusable package/example now public |
