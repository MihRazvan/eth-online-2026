...
Twelve tests pass across three suites. The two-series invariant executes 128 runs × 64 calls = 8,192 calls, zero unexpected reverts. The independent payout fuzz executes 256 cases. Tests deploy the unmodified pinned PoolManager and PositionManager and transfer actual local ERC20/native assets. Local tokens are explicitly fixtures, not authentic public-chain USDC. Position minting uses the official pre-funded PositionManager `SETTLE` path with `payerIsUser=false`; no mocked Permit2 replaces the mint path.
