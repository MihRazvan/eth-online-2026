...
Implementation feedback prepared with OpenAI Codex assistance. Source attribution is recorded in [Attribution](docs/AI_ASSISTANCE.md); this document does not imply sponsor endorsement.
...
| Authenticated historical pool endpoint | [HistoricalFeeVerifier](contracts/src/proof/HistoricalFeeVerifier.sol#L42), [BlockHashCheckpoints](contracts/src/proof/BlockHashCheckpoints.sol), [retained public witness and native oracle](docs/evidence/operations/live-series-one-native-oracle.md) |
...
5. **Include operational historical-proof guidance.** Our tested public Sepolia provider served tip proofs but rejected even tip-minus-one during probing. This is a provider observation, not a Uniswap defect. Subsequent qualification of the configured project RPC authenticated sampled historical proofs at offsets1,128,8191; the current deployment also demonstrates [authenticated remote proof restoration](docs/evidence/operations/live-series-one-restore.json). Provider behavior must be qualified individually; a sampled success is not an availability guarantee. Integrators need durable witness retention as well as a block-hash checkpoint, with explicit failure handling for either missing artifact. A shared authenticated-growth cache may amortize work, but its first proof cost remains material.

## Measurements and limits

The actual public lifecycle funded and accepted NFT39216, traded its claims through Aqua, captured1.499999USDC, returned the NFT before allocation, and authenticated0.999999USDC for the sold window. The seller separately withdrew0.500000USDC of later fees. Public allocation used **4,396,570 gas**; the participant’s1,000-claim redemption used **122,011 gas**. [Exact receipts and accounting](docs/evidence/operations/live-series-one-lifecycle.md).
