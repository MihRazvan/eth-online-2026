...
`map_pool_context` emits `feestrip.context.v1.PoolContextBlock` for every block: chain, manager, block number/hash/parent/timestamp, Initialize, Swap, and ModifyLiquidity records. Pool IDs, log order, transaction hashes, exact decimal integer amounts, ticks, and liquidity changes survive the composition. No JavaScript floating-point token arithmetic is used. Generated Rust messages come from `prost-build`, never handwritten bindings.
...
Live provider execution is currently blocked by absent authentication. The prior root CLI attempt returned `Unauthenticated` with zero data. No successful provider stream, deployed Subgraph, or live joined analysis is claimed. See `docs/evidence/substreams.md` and `docs/evidence/graph.md`.
