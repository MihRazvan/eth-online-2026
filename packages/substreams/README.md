...
The module runs against authenticated Graph Market delivery with durable history and a deployed Studio Subgraph. [Public composition evidence](../../docs/evidence/operations/live-series-one-graph.md) records an actual nonempty-series join at the same finalized block and hash. Short windows without an initialization or prior swap observation cannot establish range occupancy.
...
A dropped connection exits the runner with the durable checkpoint retained. The continuous worker adds reconnection, backoff and persistent history under service supervision. The bounded qualification command above is a separate diagnostic tool. [Official JavaScript transport guidance](https://docs.substreams.dev/how-to-guides/sinks/stream/javascript).
