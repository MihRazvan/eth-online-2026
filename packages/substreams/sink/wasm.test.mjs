...
const pkg=createSubstream(readFileSync(new URL('../feestrip-pool-context-v0.1.0.spkg',import.meta.url)));const registry=createRegistry(pkg);
...
test('real WASM ABI + upstream protobuf + official RPC envelope sinks actual typed output',()=>{
 ...
 const output=execute(upstream.fromJson({swapEvents:[event]}));assert.equal(output.toJson().swaps[0].amount0,event.amount0);
...
// Exercises the retained published decoder itself; the block is explicitly synthetic and has no receipts.
