...
Integration hardening: each operation has a 10-second total deadline and rejects a chain head over 120 seconds old or over 30 seconds in the future. Discovery observes at most four records concurrently and rechecks withdrawal tombstones after its final canonical await. The public cancellation endpoint requires an already-known listing; it does not reserve storage for arbitrary unpublished IDs. Withdrawals of known listings remain durable and block republication.

## Local manual rehearsal

After starting dedicated loopback Anvil and running `pnpm local:reset`, start `pnpm dev:listings` in another terminal. Vite forwards `/api/listings` to port8791. The SQLite database is retained under ignored `.scratch/listings-local`; after intentionally resetting the development chain, stop this service and remove only that local database before a fresh rehearsal. Never reuse local signed records against a reset chain. Public deployments use the authenticated operations gateway and its persistent volume instead.
